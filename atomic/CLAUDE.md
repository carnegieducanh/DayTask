...
- `--bg-glass-panel-alpha` (default `0.9`) + `--bg-glass-blur` (default `20px`) định nghĩa ở `:root` đầu `App.css`, dùng thay literal trong: `--bg-glass-panel`/`--bg-glass-panel-2` (rgba alpha channel) và `backdrop-filter: blur(...)` của `.main-wrap`/`.sidebar` trong block "Background Image / Glass Mode". **KHÔNG đụng** `--bg-glass`/`--bg-glass-2` gốc (dùng cho `.modal`) — 2 hệ token tách biệt như đã note ở trên.
