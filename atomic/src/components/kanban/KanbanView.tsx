...
export default function KanbanView({ liveGoals }: Props) {
  ...
  const STATUS_CONFIG: Record<GoalStatus, { label: string; dot: string }> = {
    todo:   { label: t.kanban.status.todo,   dot: '#888780' },
    doing:  { label: t.kanban.status.doing,  dot: '#3B9FE8' },
    review: { label: t.kanban.status.review, dot: '#EF9F27' },
    done:   { label: t.kanban.status.done,   dot: '#639922' },
  };
    ...
    <>
      ...
      <div className="kanban-stats-bar">
        ...
        <div className="kanban-progress-wrap">
          <span style={{ fontSize: 13, color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>{t.kanban.yearProgress}</span>
